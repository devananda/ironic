...
class NeutronV2Network(base.NetworkInterface):
    ...
    def configure_tenant_networks(self, task):
        ...
        for port_like_obj in ports + portgroups:
            ...
            if not vif_port_id:
                LOG.warning(_LW('Node %(node)s port has no vif id in extra: '
                                '%(extra)s'),
                            {'extra': port_like_obj.extra, 'node': node.uuid})
                continue
            ...
            if isinstance(port_like_obj, objects.Portgroup):
                pg_ports = objects.Port.list_by_portgroup_id(task.context,
                                                             port_like_obj.id)
