...
class TestListNodes(test_api_base.BaseApiTest):
    ...
    def test_portgroups_subresource_link(self):
        ...
        data = self.get_json(
            '/nodes/%s' % node.uuid,
            headers={'X-OpenStack-Ironic-API-Version': '1.17'})
    ...
    def test_portgroups_subresource_link_hidden_for_older_versions(self):
        ...
        data = self.get_json(
            '/nodes/%s' % node.uuid,
            headers={'X-OpenStack-Ironic-API-Version': '1.15'})
...
class TestPatch(test_api_base.BaseApiTest):
    ...
    def test_update_to_invalid_network_interface(self):
        node = obj_utils.create_test_node(self.context,
                                          uuid=uuidutils.generate_uuid())
...
class TestPost(test_api_base.BaseApiTest):
    ...
    def test_create_node_with_invalid_network_interface(self):
        ndict = test_api_utils.post_get_test_node(
            network_interface='my_interface')
        response = self.post_json('/nodes', ndict,
                                  headers={api_base.Version.string:
                                           str(api_v1.MAX_VER)},
                                  expect_errors=True)
